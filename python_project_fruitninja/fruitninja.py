...
WIDTH = 1280
HEIGHT = 800
FPS = 4                                                #controls how often the gameDisplay should refresh. In our case, it will refresh every 1/12th second
pygame.init()
...
# draw players lives
def draw_lives(display, y, z, lives, image) :
    for i in range(lives) :
        img = pygame.image.load(image)
        img_rect = img.get_rect()       #gets the (x,y) coordinates of the cross icons (lives on the the top rightmost side)
        img_rect.x = int(y + 45 * i)    #sets the next cross icon 35pixels awt from the previous one
        img_rect.y = z                  #takes care of how many pixels the cross icon should be positioned from top of the screen
